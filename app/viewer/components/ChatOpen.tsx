...
const ChatOpen = ({editor}:props) => {
    ...
    const prompt = 
        `Eres un asistente que responde preguntas acerca del siguiente codigo:
         code: ${editor.content}`;
    ...
    return (
        <div
            className={`transition-all bg-yellow-400 text-black rounded-2xl p-2 h-[100%] w-[100%] flex flex-col`}
        >         
            ...
            <form className='flex h-14 items-center gap-1' onSubmit={handleSubmitForm}>
                ...
                />
                    ...
                    <button disabled={input.length==0} type='submit' className={` transition-all duration-500  rounded-full w-10 h-10  grid place-content-center ${input.length==0?'bg-gray-400':'bg-slate-700'}`}>
